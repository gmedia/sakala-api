...
- Sequence event/log unik per deployment. Action penerima report mengunci command dan deployment dalam transaction, lalu mengalokasikan sequence secara atomic. Setiap item report menyimpan `payload_hash`; unique `(agent_command_id, idempotency_key)` membuat retry memakai sequence yang sama dan mencegah baris ganda. Key turunan fingerprint payload dipakai bila agent belum mengirim `Idempotency-Key`.
