...
test('stop project records manual_intervention signal on new request', function (): void {
    ...
    $response = $this->postJson("/api/v1/admin/projects/{$project->id}/stop", [
        'reason' => 'Test intervention',
        'idempotency_key' => 'stop-new-request-'.Str::uuid(),
    ]);
    ...
    expect($signal->tags['action_code'])->toBe('manual_stop');
});

test('stop project does not double-record manual_intervention on idempotent retry', function (): void {
    $admin = User::factory()->create(['role' => UserRole::Admin]);
    $agent = AgentNode::factory()->create(['status' => AgentNodeStatus::Ready]);
    $project = Project::factory()->create([
        'status' => ProjectStatus::Active,
